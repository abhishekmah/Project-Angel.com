...
export function AngelList() {
  ...
  const handleAddCompany = () => {
    axios.get(`https://abhi1-app-test.herokuapp.com/company?_page=${page}&_limit=4`).then(({ data }) => {
  ...
  const handleSearch = (query) => {
    ...
    axios.get(`https://abhi1-app-test.herokuapp.com/company?q=${query}`).then(({ data }) => {
      console.log('data:', data)
      setCompany(data)
      setLoading(false)
    });
